...
public class ReedSolomon {
    ...
    public int[] isParityCorrect2(byte[][] shards, int firstByte, int byteCount) {
        // Check arguments.
        checkBuffersAndSizes(shards, firstByte, byteCount);
        ...
        // Do the checking.
        return checkSomeShards2(parityRows, shards, toCheck, parityShardCount,
                firstByte, byteCount);
    }
    ...
    private int[] checkSomeShards2(final byte [] [] matrixRows,
                                    final byte [] [] inputs,
                                    final byte [] [] toCheck,
                                    final int checkCount,
                                    final int offset,
        ...
        //Função igual à original mas devolve a shard com a informação errada de modo a esta poder ser esvaziada
        int counter = 0;
        //System.out.println(byteCount);
        int[] incorrect_bytes = new int[byteCount*inputs.length];
        for(int iByte = offset; iByte < offset + byteCount; iByte++) {
            for (int iRow = 0; iRow < checkCount; iRow++){
                byte [] matrixRow = matrixRows[iRow];
                int value = 0;
                for (int c = 0; c < dataShardCount; c++) {
                    ...
                    value ^= Galois.multiply(matrixRow[c], inputs[c][iByte]);
                    if (toCheck[iRow][iByte] != (byte) value) {
                        //incorrect_bytes[counter] = 1;
                        System.out.println("Row:"+iRow);
                        System.out.println("Column:"+iByte);
