...
byte* formular_cabecalho(int grupo_fluxo_envio2, byte tmnh_msg, byte n_envio) {
  ...
  cabecalho[0] = grupo_fluxo_byte;
  tamanho_ocupado = 1;
  //Serial.println(tipo_trama);
  cabecalho[1] = tipo_trama;
  cabecalho[2] = tmnh_msg;
...
void formar_enviar_pacote(byte* mensagem, int grupo_fluxo_envio, int tamanho_msg) {
  ...
  int len = 4 + tamanho_msg + 2;
  Serial.print("Mem livre:");
  Serial.println(ESP.getFreeHeap());
  byte pacote[len];
  //byte* pacote = (byte*) malloc(len);
  ...
  memcpy(pacote, cabec_pacote, 4);  //Muitos warnings neste tipo de operações, ver uma maneira mais "aceitável" de fazer este tipo de op.
  int tamanho_ocupado = 4;
  ...
  for (int tentativas = 0; tentativas < 3; tentativas++) {  //Acho que o que está no interiro não é a melhor maneira de fazer isto, mas revejam sff.
    byte n_reenvio_byte = byte(tentativas);
    cabec_pacote = formular_cabecalho(grupo_fluxo_envio, tamanho_msg_byte, n_reenvio_byte);
    int tamanho_retirar = tamanho_msg + 5;
    memcpy(pacote , cabec_pacote + tamanho_retirar, 4);
    ...
    for (int x = 0; x < tamanho_ocupado - 1; x++) {
      ...
      for (int i = 7; i >= 0; i--){
        ...
        Serial.println(bit_lido);
         // PARA A PRIMEIRA TESTAGEM COM O RECEPTOR, quando o receptor conseguir receber mensagens tentem sem este delay
      }
      digitalWrite(SendingPin,LOW);
      delayMicroseconds(1000);  
    }
